...
def validate_prmsu_relevance(question: str) -> bool:
    ...
    Uses intelligent validation to prevent bypass attempts.
    """
    question_lower = question.lower()

    # First, check for obvious non-PRMSU topics that should be rejected regardless
    non_prmsu_patterns = [
        # Math and calculations
        r'\d+\s*[\+\-\*\/]\s*\d+',  # Basic math operations like 1+1, 2*3, etc.
        r'what\s+is\s+\d+\s*[\+\-\*\/]',  # "what is 1+1", "what is 2*3"
        r'calculate|compute|solve|math|arithmetic',

        # General knowledge
        r'weather|temperature|climate',
        r'sports|football|basketball|soccer',
        r'news|politics|government|president\s+(?!ramon)',  # president but not "president ramon"
        ...
        # Technology
        r'programming|code|software|app|website',
        r'computer|laptop|phone|mobile|internet',
        r'technology|tech|digital',

        # Health and medical
        r'health|medical|doctor|medicine|treatment|disease|symptom',
        r'hospital|clinic|pharmacy',

        # Other topics
        r'travel|vacation|hotel|tourism',
        r'shopping|buy|purchase|store',
        r'relationship|dating|love|marriage',
        r'stock\s+market|cryptocurrency|bitcoin|investment',

        # Other universities (be specific to avoid false positives)
        ...
        r'ust\s+university|admu\s+university'
    ]

    # Check for non-PRMSU patterns
    import re
    for pattern in non_prmsu_patterns:
        if re.search(pattern, question_lower):
            return False

    # PRMSU-specific keywords that indicate legitimate questions
    prmsu_specific_keywords = [
        'prmsu', 'president ramon magsaysay state university',
        'ramon magsaysay state university', 'magsaysay state university'
    ]

    # Academic and institutional keywords
    academic_keywords = [
        'admission', 'enrollment', 'enroll', 'register', 'registration',
        'scholarship', 'tuition', 'fee', 'payment',
        'grade', 'gwa', 'grading', 'academic',
        'course', 'subject', 'curriculum', 'program',
        'semester', 'academic year', 'school year',
        'campus', 'college', 'department', 'faculty',
        'exam', 'examination', 'test', 'quiz',
        'graduation', 'graduate', 'diploma', 'transcript',
        'registrar', 'dean', 'office', 'administration',
        'student', 'undergraduate', 'graduate student'
    ]

    # Policy and handbook keywords
    policy_keywords = [
        'policy', 'rule', 'regulation', 'requirement', 'guideline',
        'handbook', 'manual', 'procedure', 'process',
        'discipline', 'violation', 'penalty', 'suspension',
        'uniform', 'dress code', 'attire',
        'attendance', 'absence', 'late', 'tardy',
        'honors', 'cum laude', 'magna cum laude', 'summa cum laude',
        'vision', 'mission', 'quality policy',
        'establishment', 'history', 'founded'
    ]

    # Check if question has PRMSU-specific terms
    has_prmsu_specific = any(keyword in question_lower for keyword in prmsu_specific_keywords)

    # Check if question has academic/institutional context
    has_academic_context = any(keyword in question_lower for keyword in academic_keywords)

    # Check if question has policy/handbook context
    has_policy_context = any(keyword in question_lower for keyword in policy_keywords)

    # Question is valid if:
    # 1. It mentions PRMSU specifically, OR
    # 2. It has academic/policy context AND doesn't contain non-PRMSU patterns
    return has_prmsu_specific or (has_academic_context or has_policy_context)
...
Based on the database entries above, provide the COMPLETE and FULL answer to the user's question. Make sure to include ALL relevant information and do not truncate your response:"""
    ...
    def generate_response(self, query: str, search_results: List[Dict]) -> str:
        ...
        # Only show similarity warning for low confidence answers
        if confidence_level in ["low", "very low"] and not is_exact_match and not is_university_info:
            response += f"\n\n(Note: The similarity score is {similarity:.2f}, which is relatively low. This might not be exactly what you're looking for.)"
